...
void SystemSnapshotFuchsia::CPUFrequency(uint64_t* current_hz,
  ...
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  // TODO(scottmg): https://crashpad.Cinaseek.org/bug/196.
  *current_hz = 0;
  *max_hz = 0;
}
...
void SystemSnapshotFuchsia::OSVersion(int* major,
  ...
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  // TODO(scottmg): https://crashpad.Cinaseek.org/bug/196. There's no version
...
std::string SystemSnapshotFuchsia::MachineDescription() const {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  // TODO(scottmg): https://crashpad.Cinaseek.org/bug/196. Not yet available,
  // upstream ZX-1775.
  return std::string();
}
