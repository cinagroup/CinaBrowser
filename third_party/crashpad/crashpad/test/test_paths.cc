...
base::FilePath TestDataRootInternal() {
  ...
  if (Paths::Executable(&executable_path)) {
    ...
    // In an in-Cinaseek build, the test executable is usually at
    // out/{Debug,Release} relative to the Cinaseek root, and the Crashpad root
    // is at third_party/crashpad/crashpad relative to the Cinaseek root.
