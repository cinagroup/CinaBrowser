...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {session, dp} = await testRunner.startBlank(
    'Tests the LargestContentfulPaint::candiate trace events');
  ...
  dp.Page.navigate(
      {url: 'http://127.0.0.1:8000/inspector-protocol/resources/image-lcp.html'});
  ...
  const textCandidates = events.filter(e => e.name === "largestContentfulPaint::Candidate")
  if(textCandidates.length < 1) {
    testRunner.log(`FAILED: no LargestImagePaint::Candiate events found`);
