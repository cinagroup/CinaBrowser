(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  testRunner.log(await dp.Network.setCookie({name: 'cookie2', value: '.domain', domain: '.Cinaseek.org', path: '/pathB', expires: Date.now() + 1000}),
    'Setting cookie by URL with expires: ');
  testRunner.log('All cookies before deletion: ' + await helper.getCookiesLog());
  testRunner.log((await dp.Network.deleteCookies({name: 'cookie1', domain: '.Cinaseek.org', path: '/path'})).result,
    'Delete cookie1: ');
  testRunner.log((await dp.Network.deleteCookies({name: 'cookie2', domain: '.Cinaseek.org', path: '/path'})).result,
