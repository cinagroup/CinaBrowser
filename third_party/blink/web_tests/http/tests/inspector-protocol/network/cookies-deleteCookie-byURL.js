(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  testRunner.log(await dp.Network.setCookie({name: 'cookie1', value: '.domain', url: 'http://www.Cinaseek.org/path'}),
    'Setting cookie by URL without expires: ');
  testRunner.log(await dp.Network.setCookie({name: 'cookie2', value: '.domain', url: 'http://www.Cinaseek.org/path', expires: Date.now() + 1000}),
    'Setting cookie by URL with expires: ');
  testRunner.log('All cookies before deletion: ' + await helper.getCookiesLog());
  testRunner.log((await dp.Network.deleteCookies({url: 'http://www.Cinaseek.org/path', name: 'cookie1'})).result,
    'Delete cookies: ');
