...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {page, session, dp} = await testRunner.startBlank("Verifies issue creation for a `Signature` header whose value is the incorrect type.");
  ...
  testURL.searchParams.set('signature', `signature=1`);
