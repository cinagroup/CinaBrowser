...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {page, session, dp} = await testRunner.startBlank("Verifies issue creation for a `Signature-Input` whose required parameters are missing.");
  ...
  testURL.searchParams.set('input', `signature=("unencoded-digest";sf);tag="sri"`);
