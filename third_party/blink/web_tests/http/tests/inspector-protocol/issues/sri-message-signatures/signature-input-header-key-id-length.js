...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {page, session, dp} = await testRunner.startBlank("Verifies issue creation for a `Signature-Input` whose `keyid` is the incorrect length.");
  ...
  testURL.searchParams.set('input', `signature=("unencoded-digest";sf);keyid="BADBEEF";tag="sri"`);
