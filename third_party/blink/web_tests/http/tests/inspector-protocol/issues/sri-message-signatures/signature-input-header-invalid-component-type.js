...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {page, session, dp} = await testRunner.startBlank("Verifies issue creation for a `Signature-Input` whose components are not strings.");
  ...
  testURL.searchParams.set('input', `signature=(1);tag="ed25519-integrity"`);
