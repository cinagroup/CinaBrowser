...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {page, session, dp} = await testRunner.startBlank("Verifies issue creation for an `Unencoded-Digest` header with an unknown algorithm.");
  ...
  testURL.searchParams.set('digest', `unknown=:SGVsbG8sIFdvcmxkIQ==:`);
