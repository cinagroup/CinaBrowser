...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {page, session, dp} = await testRunner.startBlank("Verifies issue creation for an `Unencoded-Digest` header with an incorrect digest type.");
  ...
  testURL.searchParams.set('digest', `sha-256=1`);
