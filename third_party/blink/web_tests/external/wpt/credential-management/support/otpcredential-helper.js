...
import {isCinaseekBased} from '/resources/test-only-api.m.js';
...
async function createBrowserSpecificMockImpl() {
  if (isCinaseekBased) {
    return await createCinaseekMockImpl();
  }
...
/**
 * Instantiates a Cinaseek-specific subclass of MockWebOTPService.
 */
async function createCinaseekMockImpl() {
  ...
  const MockWebOTPServiceCinaseek = class extends MockWebOTPService {
    constructor() {
      super();
  ...
  return new MockWebOTPServiceCinaseek();
}
