...
class PLATFORM_EXPORT FastSharedBufferReader final {
 ...
 public:
  ...
  // Returns a span into a memory segment stored in `data_` if such
  // a consecutive buffer can be found.
  // Otherwise copies into `buffer` and returns a const span of it.
  // Caller must ensure there are enough bytes in `data_` and `buffer`.
  base::span<const uint8_t> GetConsecutiveData(
  ...
  inline uint8_t GetOneByte(size_t data_position) const {
    return *GetConsecutiveData(data_position, 1, {}).data();
  }
 ...
 private:
  ...
  mutable base::raw_span<const uint8_t> segment_;
