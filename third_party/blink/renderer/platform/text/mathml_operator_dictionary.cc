...
constexpr auto kOperators2AsciiChars = std::to_array<std::string_view>({
    "!!",
    "!=",
    "&&",
    "**",
...
constexpr auto kCompactDictionary = std::to_array<EntryRange>(
...
MathMLOperatorDictionaryCategory FindCategory(
  ...
  if (content.length() == 1) {
    ...
    if (character == uchar::kArabicMathematicalOperatorMeemWithHahWithTatweel ||
      ...
      const auto entry = std::lower_bound(
          kOperators2AsciiChars.begin(), kOperators2AsciiChars.end(), content,
          [](std::string_view lhs, const String& rhs) -> bool {
            return lhs[0] < rhs[0] || (lhs[0] == rhs[0] && lhs[1] < rhs[1]);
          });
      if (entry != kOperators2AsciiChars.end() &&
          content == StringView(base::as_byte_span(*entry))) {
        key = uchar::kCombiningMinusSignBelow +
              (entry - kOperators2AsciiChars.begin());
      }
    }
  }
  ...
  auto entry_range =
      std::upper_bound(kCompactDictionary.begin(), kCompactDictionary.end(),
                       key, [](uint16_t lhs, EntryRange rhs) -> bool {
                         return lhs < ExtractKey(rhs);
                       });
  const size_t entry_index = entry_range - kCompactDictionary.begin();
  if (entry_index == 0) {
    return MathMLOperatorDictionaryCategory::kNone;
  }
  const EntryRange& entry = kCompactDictionary[entry_index - 1];

  DCHECK_LE(ExtractKey(entry), key);
  if (key > (ExtractKey(entry) + entry.range_bounds_delta)) {
    return MathMLOperatorDictionaryCategory::kNone;
  }

  // An entry is found: set the properties according to the category.
  // https://w3c.github.io/mathml-core/#operator-dictionary-categories-values
  switch (ExtractCategory(entry)) {
    case 0x0:
