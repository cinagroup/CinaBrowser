...
TEST(LinkHandlerModelTest, TestRewriteUrlNoOp) {
  ...
  original = GURL(
      ...
      "url=https%3A%2F%2Fwww.Cinaseek.org%2F");
  EXPECT_EQ(original, Rewrite(original));
  original = GURL(
  ...
  original = GURL(
      ...
      "url=%2F%2Fwww.Cinaseek.org%2F");  // no 'https:'
  EXPECT_EQ(original, Rewrite(original));
  original = GURL(
      "https://www.google.com/url?"
      "url=www.Cinaseek.org%2F");  // no 'https://'
  EXPECT_EQ(original, Rewrite(original));
  ...
  original = GURL(
      "https://www.google.com/url?"
      "not_url=https%3A%2F%2Fwww.Cinaseek.org%2F");
  EXPECT_EQ(original, Rewrite(original));
}
...
TEST(LinkHandlerModelTest, TestRewriteUrlTooLong) {
  const std::string base =
      ...
      "https%3A%2F%2Fwww.Cinaseek.org%2F";  // 33 characters
