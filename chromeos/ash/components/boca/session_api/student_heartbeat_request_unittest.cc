...
// TODO: crbug.com/394633053 Move all tests under namespace ash::boca.
namespace {
...
class StudentHeartbeatRequestTest : public testing::Test {
 public:
  void SetUp() override {
...
TEST_F(StudentHeartbeatRequestTest, StudentHeartbeatSucceed) {
  ...
  const GaiaId gaiaId("1");
  const std::string sessionId = "session_id";
  const std::string deviceId = "device_id";
  const std::string studentGroupId = "student_group_id";
...
TEST_F(StudentHeartbeatRequestTest, StudentHeartbeatFail) {
  net::test_server::HttpRequest http_request;
  EXPECT_CALL(*request_handler(), HandleRequest(_))
      .WillOnce([&](const HttpRequest& request) {
        http_request = request;
