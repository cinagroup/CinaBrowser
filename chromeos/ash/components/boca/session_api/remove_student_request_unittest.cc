...
class RemoveStudentRequestTest : public testing::Test {
 public:
  RemoveStudentRequestTest() = default;
  void SetUp() override {
...
TEST_F(RemoveStudentRequestTest, RemoveMultipleStudentAndSucceed) {
  net::test_server::HttpRequest http_request;
  ...
  EXPECT_EQ("/v1/teachers/1/sessions/session_id/students:remove",
...
TEST_F(RemoveStudentRequestTest, RemoveEmptyStudentListAndSucceed) {
  net::test_server::HttpRequest http_request;
  EXPECT_CALL(request_handler(), HandleRequest(_))
      .WillOnce(DoAll(SaveArg<0>(&http_request),
                      Return(MockRequestHandler::CreateSuccessfulResponse())));
...
TEST_F(RemoveStudentRequestTest, RemoveStudentAndFail) {
  net::test_server::HttpRequest http_request;
  EXPECT_CALL(request_handler(), HandleRequest(_))
      .WillOnce(DoAll(SaveArg<0>(&http_request),
