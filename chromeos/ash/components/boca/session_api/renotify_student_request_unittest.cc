...
class RenotifyStudentRequestTest : public testing::Test {
 public:
  RenotifyStudentRequestTest() = default;
  void SetUp() override {
...
TEST_F(RenotifyStudentRequestTest, RenotifyMultipleStudentAndSucceed) {
  net::test_server::HttpRequest http_request;
  ...
  std::unique_ptr<RenotifyStudentRequest> request =
      std::make_unique<RenotifyStudentRequest>(request_sender(), "https://test",
                                               gaia_id, session_id,
                                               future.GetCallback());
...
TEST_F(RenotifyStudentRequestTest, RenotifyEmptyStudentListAndSucceed) {
  ...
  EXPECT_EQ(
      ...
      http_request.relative_url);
  EXPECT_EQ("application/json", http_request.headers["Content-Type"]);
  auto* contentData = "{\"users\":[]}";
  ASSERT_TRUE(http_request.has_content);
  EXPECT_EQ(contentData, http_request.content);
  EXPECT_EQ(true, result.value());
}

TEST_F(RenotifyStudentRequestTest, RenotifyStudentAndFail) {
  net::test_server::HttpRequest http_request;
  EXPECT_CALL(request_handler(), HandleRequest(_))
      .WillOnce(DoAll(SaveArg<0>(&http_request),
