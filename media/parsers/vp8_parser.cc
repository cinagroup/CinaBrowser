...
static constexpr std::array<uint8_t, kNumYModeProbs> kKeyframeYModeProbs = {
    145, 156, 163, 128};
static constexpr std::array<uint8_t, kNumUVModeProbs> kKeyframeUVModeProbs = {
    142, 114, 183};

static constexpr std::array<uint8_t, kNumYModeProbs> kDefaultYModeProbs = {
    112, 86, 140, 37};
static constexpr std::array<uint8_t, kNumUVModeProbs> kDefaultUVModeProbs = {
    162, 101, 204};

static constexpr std::array<
    std::array<std::array<std::array<uint8_t, kNumEntropyNodes>,
                          kNumPrevCoeffContexts>,
               kNumCoeffBands>,
    kNumBlockTypes>
    kDefaultCoeffProbs = {{
...
static constexpr std::array<std::array<uint8_t, kNumMVProbs>, kNumMVContexts>
    kMVUpdateProbs = {{
        {{
...
void Vp8Parser::ResetProbs() {
  curr_entropy_hdr_.coeff_probs = kDefaultCoeffProbs;

  curr_entropy_hdr_.mv_probs = kDefaultMVProbs;

  curr_entropy_hdr_.y_mode_probs = kDefaultYModeProbs;

  curr_entropy_hdr_.uv_mode_probs = kDefaultUVModeProbs;
}
...
bool Vp8Parser::ParseTokenProbs(Vp8EntropyHeader* ehdr,
  ...
  if (update_curr_probs) {
    curr_entropy_hdr_.coeff_probs = ehdr->coeff_probs;
  }
...
bool Vp8Parser::ParseIntraProbs(Vp8EntropyHeader* ehdr,
                                ...
                                bool keyframe) {
  if (keyframe) {
    ehdr->y_mode_probs = kKeyframeYModeProbs;

    ehdr->uv_mode_probs = kKeyframeUVModeProbs;
  } else {
...
bool Vp8Parser::ParseMVProbs(Vp8EntropyHeader* ehdr, bool update_curr_probs) {
  ...
  if (update_curr_probs) {
    curr_entropy_hdr_.mv_probs = ehdr->mv_probs;
  }
