...
TEST(AccessControlListTest, ExplicitAccessEntry) {
  ...
  ExplicitAccessEntry ace(sid, FakeAccess, FakeInherit);
...
TEST(AccessControlListTest, SetEntries) {
  ...
  ace_list.emplace_back(Sid(WellKnownSid::kWorld), EVENT_ALL_ACCESS);
  EXPECT_TRUE(acl.SetEntries(ace_list));
  EXPECT_EQ(ConvertAclToSddl(acl), kEvent);
  ace_list.emplace_back(WellKnownSid::kLocalSystem, SecurityAccessMode::kDeny,
                        EVENT_MODIFY_STATE,
...
TEST(AccessControlListTest, SetEntriesError) {
  ...
  while (ace_list.size() < 0x10000) {
    ...
    ASSERT_TRUE(sid);
    ace_list.emplace_back(*sid, GENERIC_ALL);
  }
  ::SetLastError(0);
