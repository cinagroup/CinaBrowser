// Copyright 2006-2009 The Cinaseek Authors
...
[[maybe_unused]] static void wrap_posix_strerror_r(
  ...
  if (rc != buf) {
    ...
    // into buf.
    buf[0] = '\0';
    PA_UNSAFE_TODO(strncat(buf, rc, len - 1));
  }
...
[[maybe_unused]] static void wrap_posix_strerror_r(
  ...
  if (result == 0) {
    ...
    PA_UNSAFE_TODO(snprintf(buf, len, "Error %d while retrieving error %d",
                            strerror_error, err));
  }
  errno = old_errno;
}
