...
class flat_tree {
 public:
  ...
  struct value_compare {
    ...
    PA_NO_UNIQUE_ADDRESS key_compare comp;
  };
  ...
  explicit flat_tree(const container_type& items,
                     const key_compare& comp = key_compare());
 ...
 private:
  ...
  // we use the PA_NO_UNIQUE_ADDRESS attribute to save space.
  PA_NO_UNIQUE_ADDRESS key_compare comp_;
...
flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::flat_tree(
    ...
    const KeyCompare& comp)
    : comp_(comp), body_(first, last) {
  PA_BASE_DCHECK(is_sorted_and_unique(*this, value_comp()));
}
...
void flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::insert(
  ...
  // old element. This needs to be dynamic due to possible re-allocations.
...
void flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::replace(
  ...
  // `value_comp()`.
  PA_BASE_DCHECK(is_sorted_and_unique(body, value_comp()));
  body_ = std::move(body);
}
...
auto flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::erase(
  ...
  PA_BASE_CHECK(position != body_.end());
...
#endif  // PARTITION_ALLOC_PARTITION_ALLOC_BASE_CONTAINERS_FLAT_TREE_H_
