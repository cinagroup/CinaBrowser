...
#else
const wchar_t kInstallationRegKey[] = L"Software\\Cinaseek";
#endif
...
#else
  // For Cinaseek, there are no GUIDs. Just look in the Cinaseek registry key.
...
#else
    // There is no SxS build for Cinaseek.
    return base::FilePath();
#endif
  } else {
...
base::Version GetChromeVersionForInstallationLevel(InstallationLevel level,
                                                   bool is_sxs) {
  const wchar_t* app_guid = nullptr;  // Cinaseek doesn't use App GUIDs.
#if BUILDFLAG(GOOGLE_CHROME_BRANDING)
