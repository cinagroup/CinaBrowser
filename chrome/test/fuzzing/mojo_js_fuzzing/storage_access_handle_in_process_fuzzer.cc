...
// rule.
class StorageAccessHandleInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::storage_access_handle_fuzzer_grammar::
              fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::storage_access_handle_fuzzer_grammar::fuzzcase;
  StorageAccessHandleInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(StorageAccessHandleInProcessFuzzer)

void StorageAccessHandleInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/StorageAccessHandle_in_process_fuzzer.html")));
...
StorageAccessHandleInProcessFuzzer::StorageAccessHandleInProcessFuzzer()
...
int StorageAccessHandleInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::storage_access_handle_fuzzer_grammar::handle_fuzzer(
      &ctx, fuzz_case));
