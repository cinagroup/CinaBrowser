...
#include "chrome/test/fuzzing/mojo_js_fuzzing/clipboard_host_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/clipboard_host_fuzzer_grammar.pb.h"
...
// rule.
class ClipboardHostInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::clipboard_host_fuzzer_grammar::fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::clipboard_host_fuzzer_grammar::fuzzcase;
  ClipboardHostInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(ClipboardHostInProcessFuzzer)

void ClipboardHostInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/ClipboardHost_in_process_fuzzer.html")));
...
ClipboardHostInProcessFuzzer::ClipboardHostInProcessFuzzer()
...
int ClipboardHostInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(
      domatolpm::clipboard_host_fuzzer_grammar::handle_fuzzer(&ctx, fuzz_case));
