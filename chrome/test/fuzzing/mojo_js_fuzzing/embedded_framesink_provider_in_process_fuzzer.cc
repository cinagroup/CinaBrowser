...
#include "chrome/test/fuzzing/mojo_js_fuzzing/embedded_framesink_provider_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/embedded_framesink_provider_fuzzer_grammar.pb.h"
...
// rule.
class EmbeddedFrameSinkProviderInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::embedded_framesink_provider_fuzzer_grammar::
              fuzzcase> {
 public:
  using FuzzCase = domatolpm::generated::
      embedded_framesink_provider_fuzzer_grammar::fuzzcase;
  EmbeddedFrameSinkProviderInProcessFuzzer();
...
void EmbeddedFrameSinkProviderInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/EmbeddedFrameSinkProvider_in_process_fuzzer.html")));
...
int EmbeddedFrameSinkProviderInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::embedded_framesink_provider_fuzzer_grammar::handle_fuzzer(
      &ctx, fuzz_case));
