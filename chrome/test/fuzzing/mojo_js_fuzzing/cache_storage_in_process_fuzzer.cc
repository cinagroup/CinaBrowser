...
#include "chrome/test/fuzzing/mojo_js_fuzzing/cache_storage_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/cache_storage_fuzzer_grammar.pb.h"
...
// rule.
class CacheStorageInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::cache_storage_fuzzer_grammar::fuzzcase> {
 public:
  using FuzzCase = domatolpm::generated::cache_storage_fuzzer_grammar::fuzzcase;
  CacheStorageInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(CacheStorageInProcessFuzzer)

void CacheStorageInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/CacheStorage_in_process_fuzzer.html")));
...
CacheStorageInProcessFuzzer::CacheStorageInProcessFuzzer()
...
int CacheStorageInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(
      domatolpm::cache_storage_fuzzer_grammar::handle_fuzzer(&ctx, fuzz_case));
