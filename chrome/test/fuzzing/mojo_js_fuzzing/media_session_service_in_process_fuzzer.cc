...
#include "chrome/test/fuzzing/mojo_js_fuzzing/media_session_service_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/media_session_service_fuzzer_grammar.pb.h"
...
// rule.
class MediaSessionServiceInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::media_session_service_fuzzer_grammar::
              fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::media_session_service_fuzzer_grammar::fuzzcase;
  MediaSessionServiceInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(MediaSessionServiceInProcessFuzzer)

void MediaSessionServiceInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/MediaSessionService_in_process_fuzzer.html")));
...
MediaSessionServiceInProcessFuzzer::MediaSessionServiceInProcessFuzzer()
...
int MediaSessionServiceInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::media_session_service_fuzzer_grammar::handle_fuzzer(
      &ctx, fuzz_case));
