...
// rule.
class BucketManagerHostInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::bucket_manager_host_fuzzer_grammar::fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::bucket_manager_host_fuzzer_grammar::fuzzcase;
  BucketManagerHostInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(BucketManagerHostInProcessFuzzer)

void BucketManagerHostInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/BucketManagerHost_in_process_fuzzer.html")));
...
BucketManagerHostInProcessFuzzer::BucketManagerHostInProcessFuzzer()
...
int BucketManagerHostInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::bucket_manager_host_fuzzer_grammar::handle_fuzzer(
      &ctx, fuzz_case));
