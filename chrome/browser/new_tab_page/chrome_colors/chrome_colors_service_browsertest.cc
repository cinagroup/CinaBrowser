 ...
 protected:
  ...
  void SetUpOnMainThread() override {
    ...
    chrome_colors_service_ =
        chrome_colors::ChromeColorsFactory::GetForProfile(GetProfile());
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService,
                       ApplyAndConfirmAutogeneratedTheme) {
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService,
                       ApplyAndRevertAutogeneratedTheme) {
  ThemeService* theme_service =
      ThemeServiceFactory::GetForProfile(GetProfile());
  ASSERT_TRUE(theme_service->UsingDefaultTheme());
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService,
                       ApplyAndConfirmDefaultTheme_withPreviousTheme) {
  ThemeService* theme_service =
      ThemeServiceFactory::GetForProfile(GetProfile());
  SkColor prev_theme_color = SkColorSetRGB(200, 0, 200);
  theme_service->BuildAutogeneratedThemeFromColor(prev_theme_color);
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService,
                       ApplyAndRevertDefaultTheme_withPreviousTheme) {
  ThemeService* theme_service =
      ThemeServiceFactory::GetForProfile(GetProfile());
  SkColor prev_theme_color = SkColorSetRGB(200, 0, 200);
  theme_service->BuildAutogeneratedThemeFromColor(prev_theme_color);
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService, RevertThemeChangesForTab) {
  ThemeService* theme_service =
      ThemeServiceFactory::GetForProfile(GetProfile());
  ASSERT_TRUE(theme_service->UsingDefaultTheme());
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService, RecordColorOnLoadHistogram) {
...
IN_PROC_BROWSER_TEST_F(TestChromeColorsService,
                       RecordDynamicColorOnLoadHistogram) {
  constexpr size_t kTestColorIndex = 3;
