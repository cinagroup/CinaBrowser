...
class PriorityBoostGpuBrowserNetworkPolicyTest : public GraphTestHarness {
 public:
  void SetUp() override {
    GraphTestHarness::SetUp();
    // Pass the policy to the graph.
    graph()->PassToGraph(
        std::make_unique<PriorityBoostGpuBrowserNetworkPolicy>());
...
TEST_F(PriorityBoostGpuBrowserNetworkPolicyTest, BrowserProcess) {
...
TEST_F(PriorityBoostGpuBrowserNetworkPolicyTest, NetworkProcess) {
  auto process_node =
...
TEST_F(PriorityBoostGpuBrowserNetworkPolicyTest, GpuProcess) {
  auto process_node = CreateBrowserChildProcessNode(content::PROCESS_TYPE_GPU);
  process_node->SetProcess(base::Process::Current(), base::TimeTicks::Now());
  ASSERT_TRUE(process_node->GetProcess().IsValid());

  // Boost should not be disabled for the GPU process. The second argument
  // to this function is set to true if boosting is *disabled*.
  EXPECT_TRUE(IsProcessPriorityBoostEnabled(process_node.get()));
}

TEST_F(PriorityBoostGpuBrowserNetworkPolicyTest, RendererProcess) {
...
TEST_F(PriorityBoostGpuBrowserNetworkPolicyTest, OtherProcess) {
  auto process_node =
      CreateBrowserChildProcessNode(content::PROCESS_TYPE_UTILITY);
  process_node->SetProcess(base::Process::Current(), base::TimeTicks::Now());
  ASSERT_TRUE(process_node->GetProcess().IsValid());
