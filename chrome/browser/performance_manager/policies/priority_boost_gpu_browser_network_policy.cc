...
PriorityBoostGpuBrowserNetworkPolicy::PriorityBoostGpuBrowserNetworkPolicy() =
    default;
...
void PriorityBoostGpuBrowserNetworkPolicy::OnPassedToGraph(Graph* graph) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  graph->AddProcessNodeObserver(this);
  CHECK(graph->HasOnlySystemNode());
}

void PriorityBoostGpuBrowserNetworkPolicy::OnTakenFromGraph(Graph* graph) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  graph->RemoveProcessNodeObserver(this);
}

void PriorityBoostGpuBrowserNetworkPolicy::OnProcessNodeAdded(
    const ProcessNode* process_node) {
  ApplyPolicy(process_node);
}

void PriorityBoostGpuBrowserNetworkPolicy::OnProcessLifetimeChange(
    const ProcessNode* process_node) {
  ApplyPolicy(process_node);
}

void PriorityBoostGpuBrowserNetworkPolicy::ApplyPolicy(
    const ProcessNode* process_node) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (process_node->GetProcessType() == content::PROCESS_TYPE_BROWSER ||
      process_node->GetProcessType() == content::PROCESS_TYPE_GPU ||
