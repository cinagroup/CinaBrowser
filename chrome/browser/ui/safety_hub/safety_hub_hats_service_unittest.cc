...
class SafetyHubHatsServiceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    // directly control how the identity test environment will be used.
    ...
    PasswordStatusCheckService* pass_service =
    ...
    pass_service->UpdateInsecureCredentialCountAsync();
    RunUntilIdle();

    // Ensure that after the setup, the overall state is safe.
    ASSERT_EQ(service()->GetOverallState(),
...
TEST_F(SafetyHubHatsServiceTest, GetOverallState_SafeBrowsingInfo) {
  ...
  EXPECT_EQ(service()->GetOverallState(),
            safety_hub::SafetyHubCardState::kInfo);
}

TEST_F(SafetyHubHatsServiceTest, GetOverallState_NotificationPermission) {
...
TEST_F(SafetyHubHatsServiceTest, GetOverallState_UnusedSitePermissions) {
...
TEST_F(SafetyHubHatsServiceTest, GetOverallState_Extension) {
  ...
  extensions::CWSInfoServiceFactory::GetInstance()->SetTestingFactory(
      profile(),
      base::BindRepeating(&SafetyHubHatsServiceTest::SetMockCWSInfoService,
...
TEST_F(SafetyHubHatsServiceTest, GetOverallState_Password) {
...
TEST_F(SafetyHubHatsServiceTest, GetOverallState_Multi) {
