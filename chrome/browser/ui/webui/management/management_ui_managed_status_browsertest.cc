...
constexpr char kPromotionBannerVisibilityJavaScript[] = R"(
  (function() {
    ...
    if (managementUi && managementUi.shouldShowPromotion_) {
      return managementUi.shouldShowPromotion_ ? 'visible' : 'hidden';
    }
    return 'hidden';
  })();
)";
...
class ManagementUIManagedStatusTest
    : public PlatformBrowserTest,
      public ::testing::WithParamInterface<bool> {
 public:
  ManagementUIManagedStatusTest()
    ...
    embedded_test_server_.RegisterRequestHandler(base::BindRepeating(
        ...
        base::BindRepeating(
            &ManagementUIManagedStatusTest::HandleUserInfoRequest,
            base::Unretained(this))));
    embedded_test_server_.RegisterRequestHandler(base::BindRepeating(
        &net::test_server::HandlePrefixedRequest, "/devicemanagement",
        base::BindRepeating(
            &ManagementUIManagedStatusTest::HandleDeviceManagementRequest,
            base::Unretained(this))));
    embedded_test_server_.RegisterRequestHandler(base::BindRepeating(
        &FakeGaia::HandleRequest, base::Unretained(&fake_gaia_)));
    scoped_feature_list_.InitWithFeatureState(
        features::kEnableManagementPromotionBanner, GetParam());
  }
  ManagementUIManagedStatusTest(const ManagementUIManagedStatusTest&) = delete;
  ManagementUIManagedStatusTest& operator=(
      const ManagementUIManagedStatusTest&) = delete;

  ~ManagementUIManagedStatusTest() override = default;
 ...
 protected:
  ...
  void SetupAndListenForPromotion() {
    ...
    if (is_feature_enabled() &&
      ...
      handler->AddManagementPromotionObserver(&promotion_observer);
      EXPECT_TRUE(promotion_observer.Wait());
      handler->RemoveManagementPromotionObserver(&promotion_observer);
    }
  }
...
IN_PROC_BROWSER_TEST_P(ManagementUIManagedStatusTest,
  ...
  histogram_tester.ExpectBucketCount(
      "Enterprise.ManagementPromotionBannerDisplayed", expected_bucket, 1);
}
