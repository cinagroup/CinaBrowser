...
void TabResourceUsage::SetMemoryUsage(base::ByteSize memory_usage) {
...
void TabResourceUsageTabHelper::PrimaryPageChanged(content::Page&) {
  ...
  resource_usage_->SetMemoryUsage(base::ByteSize(0));
  NotifyResourceUsageChanged();
}

base::ByteSize TabResourceUsageTabHelper::GetMemoryUsage() {
  return resource_usage_->memory_usage();
}

void TabResourceUsageTabHelper::SetMemoryUsage(base::ByteSize memory_usage) {
