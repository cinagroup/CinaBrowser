...
class XzExtractorTest : public testing::Test {
 public:
  void SetUp() override { ASSERT_TRUE(temp_dir_.CreateUniqueTempDir()); }

  XzExtractor* CreateExtractor(ExtractionProperties properties) {
    return XzExtractor::CreateForTesting(std::move(properties));
  }

  void RunOnProgress(XzExtractor* extractor,
                     uint64_t total,
                     uint64_t progress) {
  ...
  void DeleteExtractor(XzExtractor* extractor) {
    // XzExtractor manages its own lifetime and therefore has a private
...
// Tests that the XzExtractor fails gracefully if it receives a progress update
// with 0 total bytes. This guards against a potential divide-by-zero crash in
// the progress calculation logic.
TEST_F(XzExtractorTest, ZeroTotalBytes) {
  ...
  // Create extractor.
  XzExtractor* extractor = CreateExtractor(std::move(properties));
...
// Tests that the XzExtractor fails gracefully and reports a bad message if it
...
TEST_F(XzExtractorTest, ExceedsInt64MaxTotalBytes) {
  ExtractionProperties properties;
  properties.temp_dir_path = temp_dir();
...
// values.
TEST_F(XzExtractorTest, InvalidProgressBytes) {
  ExtractionProperties properties;
  properties.temp_dir_path = temp_dir();
