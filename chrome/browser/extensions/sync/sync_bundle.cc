...
void SyncBundle::PushSyncDeletion(const ExtensionId& extension_id,
  ...
  PushSyncChanges(syncer::SyncChangeList(
      1, syncer::SyncChange(FROM_HERE, syncer::SyncChange::ACTION_DELETE,
                            sync_data)));
}
...
void SyncBundle::AddPendingExtensionData(
  ...
  pending_sync_data_.insert(
      std::make_pair(extension_sync_data.id(), extension_sync_data));
}
...
syncer::SyncChange SyncBundle::CreateSyncChange(
  ...
  return syncer::SyncChange(FROM_HERE,
                            HasSyncedExtension(extension_id)
