...
class UpdateMetricsProviderBrowserTest : public InProcessBrowserTest {
 protected:
  void SetUpOnMainThread() override {
    ...
    detector_ = UpgradeDetector::GetInstance();
    // Reset state.
    detector_->set_upgrade_available(UpgradeDetector::UPGRADE_AVAILABLE_NONE);
  }
  ...
  void SetUpgradeAvailable() {
    detector_->set_upgrade_available(
        UpgradeDetector::UPGRADE_AVAILABLE_REGULAR);
    detector_->set_upgrade_detected_time(base::Time::Now() - base::Hours(2));
  }
