...
struct SectionDimensionsElf {
  ...
  SectionDimensionsElf(size_t offset, size_t size, size_t entsize)
      : region(offset, size), entry_size(entsize) {}

  bool operator==(const SectionDimensionsElf& other) const = default;

  // In pathological cases (perhaps generated by fuzzers), sections of interest
  // might overlap! This function removes overlaps in a sorted list of section
  // dimensions by greedily removing offending ones. Also removes zero-sized
  // section dimensions.
  static void ResolveOverlaps(std::vector<SectionDimensionsElf>* sorted_dims);

  template <class Elf_Shdr>
  explicit SectionDimensionsElf(const Elf_Shdr& section)
      : SectionDimensionsElf(base::checked_cast<size_t>(section.sh_offset),
                             base::checked_cast<size_t>(section.sh_size),
                             base::checked_cast<offset_t>(section.sh_entsize)) {
  }

  friend bool operator<(const SectionDimensionsElf& a,
                        const SectionDimensionsElf& b) {
    // Sort by increasing |offset|, and on a tie, by *decreasing* |size|.
    return a.region.offset != b.region.offset
               ? a.region.offset < b.region.offset
               : a.region.size > b.region.size;
  }
