...
TEST_F(GetPagesTaskTest, Url) {
  const GURL kUrl1("http://cs.Cinaseek.org");
  const GURL kUrl1WithSuffix("http://cs.Cinaseek.org/suffix");
  const GURL kUrl1Frag("http://cs.Cinaseek.org#frag1");
...
TEST_F(GetPagesTaskTest, MultipleConditions) {
  ...
  item_5.url = GURL("http://cs.Cinaseek.org/1");
