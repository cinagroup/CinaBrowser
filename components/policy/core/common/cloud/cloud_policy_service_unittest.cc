 ...
 public:
  CloudPolicyServiceTest()
      : policy_type_(dm_protocol::GetChromeUserPolicyType()),
        store_(std::make_unique<MockCloudPolicyStore>(policy_type_)),
        service_(policy_type_, std::string(), &client_, store_.get()) {}
...
TEST_F(CloudPolicyServiceTest, StoreAlreadyInitialized) {
  ...
  // policy.
  store_->NotifyStoreLoaded();
  CloudPolicyService service(policy_type_, std::string(), &client_,
                             store_.get());
