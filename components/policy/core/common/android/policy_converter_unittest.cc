...
class PolicyConverterTest : public testing::Test {
 ...
 protected:
  ...
  std::string Convert(base::Value value, const Schema& value_schema) {
...
TEST_F(PolicyConverterTest, ConvertToBoolValue) {
  ...
  EXPECT_EQ("true", Convert(base::Value(true), bool_schema));
  EXPECT_EQ("false", Convert(base::Value(false), bool_schema));
  EXPECT_EQ("true", Convert(base::Value("true"), bool_schema));
  EXPECT_EQ("false", Convert(base::Value("false"), bool_schema));
  EXPECT_EQ("\"narf\"", Convert(base::Value("narf"), bool_schema));
  EXPECT_EQ("false", Convert(base::Value(0), bool_schema));
  EXPECT_EQ("true", Convert(base::Value(1), bool_schema));
  EXPECT_EQ("true", Convert(base::Value(42), bool_schema));
  EXPECT_EQ("true", Convert(base::Value(-1), bool_schema));
  EXPECT_EQ("\"1\"", Convert(base::Value("1"), bool_schema));
  EXPECT_EQ("{}", Convert(base::Value(base::Value::Type::DICT), bool_schema));
}
...
TEST_F(PolicyConverterTest, ConvertToIntValue) {
  ...
  EXPECT_EQ("23", Convert(base::Value(23), int_schema));
  EXPECT_EQ("42", Convert(base::Value("42"), int_schema));
  EXPECT_EQ("-1", Convert(base::Value("-1"), int_schema));
  EXPECT_EQ("\"poit\"", Convert(base::Value("poit"), int_schema));
  EXPECT_EQ("false", Convert(base::Value(false), int_schema));
}
...
TEST_F(PolicyConverterTest, ConvertToDoubleValue) {
  ...
  EXPECT_EQ("3", Convert(base::Value(3), double_schema));
  EXPECT_EQ("3.14", Convert(base::Value(3.14), double_schema));
  EXPECT_EQ("2.71", Convert(base::Value("2.71"), double_schema));
  EXPECT_EQ("\"zort\"", Convert(base::Value("zort"), double_schema));
  EXPECT_EQ("true", Convert(base::Value(true), double_schema));
}
...
TEST_F(PolicyConverterTest, ConvertToStringValue) {
  ...
  EXPECT_EQ("\"troz\"", Convert(base::Value("troz"), string_schema));
  EXPECT_EQ("4711", Convert(base::Value(4711), string_schema));
}
...
TEST_F(PolicyConverterTest, ConvertToListValue) {
  ...
  list.Append("bar");
  EXPECT_EQ("[\"foo\",\"bar\"]",
            Convert(base::Value(std::move(list)), list_schema));
  EXPECT_EQ("[\"baz\",\"blurp\"]",
            Convert(base::Value("[\"baz\", \"blurp\"]"), list_schema));
  EXPECT_EQ("[\"hurz\"]", Convert(base::Value("hurz"), list_schema));
...
TEST_F(PolicyConverterTest, ConvertToDictValue) {
  ...
  dict.Set("thx", 1138);
  EXPECT_EQ("{\"thx\":1138}",
            Convert(base::Value(std::move(dict)), dict_schema));
