...
TEST(ContextualTaskTest, AddThread) {
  ...
  task.AddThread(
      Thread(type, server_id, title, last_turn_time, conversation_turn_id));
  ...
  EXPECT_EQ(task.GetThread()->last_turn_time.InMillisecondsSinceUnixEpoch(),
            last_turn_time);
  ...
  int64_t last_turn_time_2 = 2;
  task.AddThread(Thread(type, server_id_2, title_2, last_turn_time_2,
                        conversation_turn_id_2));
  EXPECT_TRUE(task.GetThread().has_value());
  EXPECT_EQ(task.GetThread()->type, type);
