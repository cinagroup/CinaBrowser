...
struct VIZ_COMMON_EXPORT ReturnedResource {
  ...
  // by multiple compositor frames submitted to the parent compositor. |count|
...
// ReturnedResourceViz is the counterpart of ReturnedResource used only in the
// GPU process. This separation allows ReturnedResource to use an opaque
// struct to hide SyncTokens from the client, whereas the GPU process can still
// use SyncTokens directly.
struct VIZ_COMMON_EXPORT ReturnedResourceViz {
  ReturnedResourceViz(ResourceId id,
  ...
  ReturnedResourceViz();
  ~ReturnedResourceViz();
  ReturnedResourceViz(ReturnedResourceViz&& other);
  ReturnedResourceViz& operator=(ReturnedResourceViz&& other);

  ReturnedResourceViz(const ReturnedResourceViz& other) = delete;
  ReturnedResourceViz& operator=(const ReturnedResourceViz& other) = delete;
