...
class WebAuthnRateLimiterSlideWindowTest : public ::testing::Test {
 protected:
...
TEST_F(WebAuthnRateLimiterSlideWindowTest, SingularEvent) {
...
TEST_F(WebAuthnRateLimiterSlideWindowTest, SteadyEventsStream) {
...
TEST_F(WebAuthnRateLimiterSlideWindowTest, RandomizedEventsStream) {
  for (size_t i = 0; i < 2 * kBucketCount; ++i) {
    ASSERT_TRUE(rate_limiter_.Acquire(kTotalSize / kBucketCount));
    if (i >= kBucketCount) {
      ASSERT_FALSE(rate_limiter_.Acquire(1u));
...
TEST_F(WebAuthnRateLimiterSlideWindowTest, SparseEventsStream) {
  for (size_t i = 0; i < 2 * kBucketCount; ++i) {
