...
class SavePasswordsEphemeralModuleTest : public testing::Test {
 public:
  SavePasswordsEphemeralModuleTest() = default;
  ~SavePasswordsEphemeralModuleTest() override = default;

  void SetUp() override {
    Test::SetUp();
    SavePasswordsEphemeralModule::RegisterProfilePrefs(
...
TEST_F(SavePasswordsEphemeralModuleTest,
  ...
  EXPECT_TRUE(SavePasswordsEphemeralModule::IsModuleLabel(
...
TEST_F(SavePasswordsEphemeralModuleTest,
  ...
  EXPECT_FALSE(SavePasswordsEphemeralModule::IsModuleLabel("some_other_label"));
}

// Verifies that the `OutputLabels(…)` method returns the expected labels.
TEST_F(SavePasswordsEphemeralModuleTest, OutputLabelsReturnsExpectedLabels) {
  ...
  ASSERT_EQ(labels.front(), kSavePasswordsEphemeralModule);
}

// Verifies that the `GetInputs(…)` method returns the expected inputs.
TEST_F(SavePasswordsEphemeralModuleTest, GetInputsReturnsExpectedInputs) {
  auto ephemeral_module =
      std::make_unique<SavePasswordsEphemeralModule>(&pref_service_);
...
// Verifies that `ComputeCardResult(…)` shows the Save Passwords module
...
TEST_F(SavePasswordsEphemeralModuleTest,
  ...
  EXPECT_EQ(kSavePasswordsEphemeralModule, result.result_label);
}

// Verifies that `ComputeCardResult(…)` does not show the module when the
// corresponding signals are present, but the password manager is not allowed
...
TEST_F(SavePasswordsEphemeralModuleTest,
  ...
  auto card = std::make_unique<SavePasswordsEphemeralModule>(&pref_service_);

  EXPECT_TRUE(SavePasswordsEphemeralModule::IsEnabled(&pref_service_));
  ...
  card->OnShow(&pref_service_, nullptr);
  EXPECT_FALSE(SavePasswordsEphemeralModule::IsEnabled(&pref_service_));
}
