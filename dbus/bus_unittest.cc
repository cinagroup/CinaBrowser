...
TEST(BusTest, GetObjectProxy) {
  ...
  ObjectProxy* object_proxy1 =
      bus->GetObjectProxy("org.Cinaseek.TestService",
  ...
  ObjectProxy* object_proxy3 =
      bus->GetObjectProxy(
          ...
          ObjectPath("/org/Cinaseek/DifferentTestObject"));
...
TEST(BusTest, RemoveObjectProxy) {
  ...
  ASSERT_FALSE(bus->RemoveObjectProxy("org.Cinaseek.TestService",
                                      ObjectPath("/org/Cinaseek/TestObject"),
                                      base::DoNothing()));
  ...
  // at a later time.
  ASSERT_TRUE(bus->RemoveObjectProxy("org.Cinaseek.TestService",
                                     ObjectPath("/org/Cinaseek/TestObject"),
                                     base::DoNothing()));
...
TEST(BusTest, GetExportedObject) {
  ...
  ExportedObject* object_proxy1 =
      bus->GetExportedObject(ObjectPath("/org/Cinaseek/TestObject"));
  ASSERT_TRUE(object_proxy1);
...
TEST(BusTest, UnregisterExportedObject) {
  ...
  bus->UnregisterExportedObject(ObjectPath("/org/Cinaseek/TestObject"));
...
TEST(BusTest, DoubleAddAndRemoveMatch) {
  ...
  bus->AddMatch("type='signal',interface='org.Cinaseek.TestService',path='/'",
                &error);
  ...
  ASSERT_TRUE(bus->RemoveMatch(
      "type='signal',interface='org.Cinaseek.TestService',path='/'", &error));
  ASSERT_FALSE(error.IsValid());
...
TEST(BusTest, ListenForServiceOwnerChange) {
  ...
  bus->ListenForServiceOwnerChange("org.Cinaseek.TestService", callback1);
  ...
  ASSERT_TRUE(bus->RequestOwnershipAndBlock("org.Cinaseek.TestService",
                                            Bus::REQUIRE_PRIMARY));
...

  {
    ...
    std::string current_service_owner =
        bus->GetServiceOwnerAndBlock("org.Cinaseek.TestService",
                                     Bus::REPORT_ERRORS);
  ...
  bus->ListenForServiceOwnerChange("org.Cinaseek.TestService", callback2);
  base::RunLoop().RunUntilIdle();

  // Release the ownership and make sure the service owner listeners fire with
  // the right values and the right number of times.
  ASSERT_TRUE(bus->ReleaseOwnership("org.Cinaseek.TestService"));
  ...
  bus->UnlistenForServiceOwnerChange("org.Cinaseek.TestService", callback1);
  bus->UnlistenForServiceOwnerChange("org.Cinaseek.TestService", callback2);
  base::RunLoop().RunUntilIdle();
