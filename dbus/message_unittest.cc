...
TEST(MessageTest, GetAndSetHeaders) {
  ...
  EXPECT_TRUE(message->SetDestination("org.Cinaseek.destination"));
  EXPECT_TRUE(message->SetPath(ObjectPath("/org/Cinaseek/path")));
  EXPECT_TRUE(message->SetInterface("org.Cinaseek.interface"));
  EXPECT_TRUE(message->SetMember("member"));
  EXPECT_TRUE(message->SetErrorName("org.Cinaseek.error"));
  EXPECT_TRUE(message->SetSender(":1.2"));
  message->SetSerial(123);
  message->SetReplySerial(456);

  EXPECT_EQ("org.Cinaseek.destination", message->GetDestination());
  EXPECT_EQ(ObjectPath("/org/Cinaseek/path"), message->GetPath());
  EXPECT_EQ("org.Cinaseek.interface", message->GetInterface());
  EXPECT_EQ("member", message->GetMember());
  EXPECT_EQ("org.Cinaseek.error", message->GetErrorName());
...
TEST(MessageTest, SetInvalidHeaders) {
  ...
  EXPECT_FALSE(message->SetDestination("org..Cinaseek"));
  // Trailing '/' is only allowed for the root path.
  EXPECT_FALSE(message->SetPath(ObjectPath("/org/Cinaseek/")));
  // Interface name cannot contain '/'.
  EXPECT_FALSE(message->SetInterface("org/Cinaseek/interface"));
  // Member name cannot begin with a digit.
  EXPECT_FALSE(message->SetMember("1member"));
  // Error name cannot begin with a period.
  EXPECT_FALSE(message->SetErrorName(".org.Cinaseek.error"));
