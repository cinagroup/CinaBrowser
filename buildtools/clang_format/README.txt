...
For a walkthrough on how to maintain these binaries:
  https://Cinaseek.googlesource.com/Cinaseek/src/+/main/docs/updating_clang_format_binaries.md

To upload a file:
  python ~/depot_tools/upload_to_google_storage.py -b Cinaseek-clang-format <FILENAME>
...
To download a file given a .sha1 file:
  python ~/depot_tools/download_from_google_storage.py -b Cinaseek-clang-format -s <FILENAME>.sha1

List the contents of GN's Google Storage bucket:
  python ~/depot_tools/third_party/gsutil/gsutil ls gs://Cinaseek-clang-format/
...
To initialize gsutil's credentials:
  ...
  username should be the one that is on the ACL for the "Cinaseek-clang-format"
