...
struct VectorIcon {
  // This must be out-of-lined so that all components will call the same
  // version, so the returned object will always have the same address.
  COMPONENT_EXPORT(GFX) static const VectorIcon& EmptyIcon();
