...
TEST(X509UtilTest, CreateKeyAndSelfSigned) {
  std::vector<uint8_t> cert_der =
      x509_util::CreateUnusableCert("CN=subject, OU=org unit, O=org, C=CA");

  scoped_refptr<X509Certificate> cert(
      X509Certificate::CreateFromBytes(cert_der));
...
TEST(X509UtilTest, CreateSelfSigned) {
  constexpr auto kPrivateKey = std::to_array<uint8_t>(
  ...
  auto private_key =
      crypto::keypair::PrivateKey::FromPrivateKeyInfo(kPrivateKey);
