...
TEST_P(WebSocketMultiProtocolStreamCreateTest, SubProtocolIsUsed) {
  ...
  sub_protocols.push_back("chatv11.Cinaseek.org");
  sub_protocols.push_back("chatv20.Cinaseek.org");
  CreateAndConnectStandard(
      "wss://www.example.org/testing_path", sub_protocols, {},
      {{"Sec-WebSocket-Protocol",
        "chatv11.Cinaseek.org, chatv20.Cinaseek.org"}},
      {{"Sec-WebSocket-Protocol", "chatv20.Cinaseek.org"}});
  WaitUntilConnectDone();
  ASSERT_TRUE(stream_);
  EXPECT_FALSE(has_failed());
  EXPECT_EQ("chatv20.Cinaseek.org", stream_->GetSubProtocol());
}
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, UnsolicitedSubProtocol) {
  ...
  EXPECT_EQ("Error during WebSocket handshake: "
            ...
            "if not present in request: chatv20.Cinaseek.org",
            failure_message());
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, UnmatchedSubProtocolInResponse) {
  ...
  CreateAndConnectStandard(
      ...
      {{"Sec-WebSocket-Protocol",
        "chatv11.Cinaseek.org, chatv20.Cinaseek.org"}},
      {{"Sec-WebSocket-Protocol", "chatv21.Cinaseek.org"}});
  WaitUntilConnectDone();
  EXPECT_FALSE(stream_);
  EXPECT_TRUE(has_failed());
  EXPECT_EQ("Error during WebSocket handshake: "
            "'Sec-WebSocket-Protocol' header value 'chatv21.Cinaseek.org' "
