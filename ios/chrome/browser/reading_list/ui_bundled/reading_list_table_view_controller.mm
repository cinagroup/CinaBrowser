...
#import "ios/chrome/browser/reading_list/ui_bundled/reading_list_list_item_updater.h"
...
- (void)moveItemsFromSection:(ReadingListSectionIdentifier)fromSection
  ...
  for (NSInteger row = 0; row < itemCount; ++row) {
    NSIndexPath* itemPath = [NSIndexPath indexPathForRow:row
...
- (void)moveItemsAtIndexPaths:(NSArray*)sortedIndexPaths
  ...
  void (^updates)(void) = ^{
    ...
    for (NSIndexPath* indexPath in sortedIndexPaths) {
      ...
      NSIndexPath* newIndexPath = [NSIndexPath indexPathForItem:newItemIndex++
