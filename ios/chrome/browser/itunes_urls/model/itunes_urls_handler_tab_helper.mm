...
constexpr std::string_view kLegacyITunesUrlHost = "itunes.apple.com";
constexpr std::string_view kLegacyITunesGlobalUrlHost = "geo.itunes.apple.com";

// The host for iOS applications URLs generated by itunes link maker.
constexpr std::string_view kAppUrlHost = "apps.apple.com";

constexpr std::string_view kITunesProductIdPrefix = "id";
constexpr std::string_view kITunesAppPathIdentifier = "app";
constexpr std::string_view kITunesAppBundlePathIdentifier = "app-bundle";
...
bool IsITunesProductUrl(const GURL& url) {
  if (!url.SchemeIsHTTPOrHTTPS() ||
      !(url.host() == kLegacyITunesUrlHost ||
        url.host() == kLegacyITunesGlobalUrlHost ||
        url.host() == kAppUrlHost)) {
    return false;
  }
...
NSDictionary* ExtractITunesProductParameters(const GURL& url) {
  ...
  std::string product_id =
      url.ExtractFileName().substr(kITunesProductIdPrefix.size());
...
bool ITunesUrlsHandlerTabHelper::CanHandleUrl(const GURL& url) {
  ...
  std::vector<std::string_view> path_components = base::SplitStringPiece(
