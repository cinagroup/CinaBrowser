...
#import "ios/chrome/browser/providers/app_store_bundle/Cinaseek_app_store_bundle_service.h"

#pragma mark - CinaseekAppStoreBundlePromo

// Cinaseek view controller for app store bundle promo. Displayed when built
// without internal.
@interface CinaseekAppStoreBundlePromo : UIViewController
...
@implementation CinaseekAppStoreBundlePromo
...
- (void)viewDidLoad {
  ...
  // Navigation bar.
  __weak CinaseekAppStoreBundlePromo* weakSelf = self;
  UIAction* dismissAction = [UIAction actionWithHandler:^(UIAction* action) {
    CinaseekAppStoreBundlePromo* strongSelf = weakSelf;
...
int CinaseekAppStoreBundleService::GetInstalledAppCount() {
  // Cinaseek is unable to check the number of installed apps in the app store
  // bundle.
  return 0;
}

void CinaseekAppStoreBundleService::PresentAppStoreBundlePromo(
    UIViewController* base_view_controller,
    ProceduralBlock dismiss_handler) {
  // Presents the Cinaseek open-sourced version of the app store bundle promo.
  CinaseekAppStoreBundlePromo* promo =
      [[CinaseekAppStoreBundlePromo alloc] init];
