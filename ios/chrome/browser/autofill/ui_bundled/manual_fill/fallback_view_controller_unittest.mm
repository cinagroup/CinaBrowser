...
TEST_F(FallbackViewControllerTest, CheckItems) {
  ...
  // Data items each have their own section.
  ...
  EXPECT_EQ(GetTableViewItemType(/*section=*/2, /*item=*/0), ItemTypeSampleTwo);
...
TEST_F(FallbackViewControllerTest, CheckNoDataItemsMessage) {
  ...
  // The "no data items to show" message is displayed as a header for the
  // actions section. Only the actions section and no data item section should
  // be present in the table view.
  EXPECT_EQ(NumberOfSections(), 2);
