...
void NavigationManagerImpl::Reload(ReloadType reload_type,
  ...
  // item's url. For example, the user visits www.Cinaseek.org and is then
  // redirected to m.Cinaseek.org, when the user wants to refresh the page with
  // a different configuration (e.g. user agent), the user would be expecting to
  // visit www.Cinaseek.org instead of m.Cinaseek.org.
