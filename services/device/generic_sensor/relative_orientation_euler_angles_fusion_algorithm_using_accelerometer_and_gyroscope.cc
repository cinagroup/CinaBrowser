...
bool RelativeOrientationEulerAnglesFusionAlgorithmUsingAccelerometerAndGyroscope::
  ...
  // [-PI/2, PI/2].
  double scale = std::numbers::pi / 2;

  alpha_ += gyro_z * dt;
  // Make sure |alpha_| is in [0, 2*PI).
  alpha_ = std::fmod(alpha_, 2 * std::numbers::pi);
  if (alpha_ < 0)
    alpha_ += 2 * std::numbers::pi;

  beta_ = kBias * (beta_ + gyro_x * dt) +
          (1.0 - kBias) * (accel_x * scale * norm_reciprocal);
  // Make sure |beta_| is in [-PI, PI).
  beta_ = std::fmod(beta_, 2 * std::numbers::pi);
  if (beta_ >= std::numbers::pi) {
    beta_ -= 2 * std::numbers::pi;
  } else if (beta_ < -std::numbers::pi) {
    beta_ += 2 * std::numbers::pi;
  }
  ...
  gamma_ = std::fmod(gamma_, std::numbers::pi);
  if (gamma_ >= std::numbers::pi / 2) {
    gamma_ -= std::numbers::pi;
  } else if (gamma_ < -std::numbers::pi / 2) {
    gamma_ += std::numbers::pi;
  }

  fused_reading->orientation_euler.z = base::RadToDeg(alpha_);
  fused_reading->orientation_euler.x = base::RadToDeg(beta_);
  fused_reading->orientation_euler.y = base::RadToDeg(gamma_);
